...
export default function Settings({ userId: propUserId, onBalanceChange, onClose, onOrdersCleared }) {
  ...
  const userId = propUserId || localStorage.getItem('userId') || 'default';
  ...
  const handleClearAllOrders = async () => {
    ...
    try {
      const res = await axios.delete(`/api/orders/user/${encodeURIComponent(userId)}/all`);
      setMessage({ type: 'success', text: res.data.message || 'All orders cleared successfully' });
      
      // Verify deletion by re-fetching
      const verifyRes = await axios.get(`/api/orders/${encodeURIComponent(userId)}`);
      if (verifyRes.data.length === 0) {
        setOrders([]);
        setMessage({ type: 'success', text: res.data.message || 'All orders cleared successfully' });
      } else {
        setOrders(verifyRes.data);
        setMessage({ type: 'error', text: `Clear failed - ${verifyRes.data.length} orders still remain` });
      }
      
      // Refresh the user data to get updated balance
      fetchProfile();
      if (onOrdersCleared) onOrdersCleared();
    } catch (err) {
          ...
          <div className="settings-card__header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {orders.length > 0 && (
              <button 
                ...
                style={{ fontSize: '0.85rem', padding: '6px 12px', background: 'var(--red-bg)', color: 'var(--red)', border: '1px solid var(--red)', borderRadius: '4px', cursor: 'pointer', fontWeight: '500' }}
              >
              ...
              {orders.map((order) => (
                <div key={order.id} className="settings-order-row">
                  <span className="settings-order-date">
                    {new Date(order.orderTime + 'Z').toLocaleString('en-IN', {
                      day: '2-digit',
                      month: 'short',
                      hour: '2-digit',
                      minute: '2-digit'
