...
export default function NiftyOptionsPage({ onBack, onOptionClick }) {
  const [expiries, setExpiries] = useState([]);
  const [selectedExpiry, setSelectedExpiry] = useState('');
  const [searchStrike, setSearchStrike] = useState('');
  const [spotPrice, setSpotPrice] = useState(24150);
  const [optionChain, setOptionChain] = useState([]);
  const [loading, setLoading] = useState(false);
  const tableRef = useRef(null);

  const strikes = useMemo(() => optionChain.map(item => item.strike), [optionChain]);
  const { getLiveLTP } = useLiveOptionPrices('NIFTY', strikes);

  useEffect(() => {
    const fetchExpiries = async () => {
      try {
        const res = await axios.get('/api/options/expiries/NIFTY');
  ...
  const fetchChain = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`/api/options/chain/NIFTY?expiry=${selectedExpiry}`);
    ...
    <div className="options-page">
      <header className="options-page__header">
        <div className="options-page__header-left">
          ...
          <h1 className="options-page__title">NIFTY OPTIONS</h1>
        </div>
