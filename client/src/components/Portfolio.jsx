...
export default function Portfolio({ data, onRefresh }) {
  ...
  const handleQuickSell = async (symbol, qtyToSell) => {
    try {
      const qty = qtyToSell === '' ? 1 : qtyToSell;
      const isOption = data[symbol]?.type === 'OPTION';
      if (isOption) {
        const lotSize = getLotSize(data[symbol]?.index);
        const contracts = qty * lotSize;
        await axios.post('/api/options/trade', {
          contract: symbol,
          action: 'sell',
          quantity: contracts,
          premium: data[symbol].currentPrice,
          strike: data[symbol].strike,
          type: data[symbol].type,
          index: data[symbol].index,
          expiry: data[symbol].expiry
        });
      } else {
      ...
      addToast({ type: 'error', message: 'Sell failed: ' + (err.response?.data?.error || err.message) });
    }
  };

  const getMaxQty = (pos) => pos.type === 'OPTION' ? (pos.lots || 1) : (pos.qty || 1);

  const getSellQty = (symbol, maxQty) => {
    const raw = sellQty[symbol];
    if (raw === '') return '';
  ...
  const updateSellQty = (symbol, value, maxQty) => {
    if (value === '') { setSellQty((prev) => ({ ...prev, [symbol]: '' })); return; }
    ...
    <aside className="portfolio-panel">
      ...
      <section className="portfolio-holdings-card">
        <div className="portfolio-section-heading portfolio-section-heading--row">
          <div>
            <h3 className="portfolio-section-heading__headline">PORTFOLIO HOLDINGS</h3>
          </div>
        ...
        {holdings === 0 ? (
          ...
          <div className="portfolio-table-wrap">
            <table className="portfolio-table">
              <thead>
                <tr>
                  <th className="portfolio-col-highlight" onClick={() => handleSort('symbol')}>Symbol {sortMarker('symbol')}</th>
                  <th className="portfolio-col-highlight" onClick={() => handleSort('qty')}>Qty {sortMarker('qty')}</th>
                  <th className="portfolio-col-highlight" onClick={() => handleSort('avgPrice')}>Avg {sortMarker('avgPrice')}</th>
                  <th className="portfolio-col-highlight" onClick={() => handleSort('currentPrice')}>Current {sortMarker('currentPrice')}</th>
                  <th className="portfolio-col-highlight" onClick={() => handleSort('value')}>Value {sortMarker('value')}</th>
                  <th className="portfolio-col-highlight" onClick={() => handleSort('pnl')}>P&amp;L {sortMarker('pnl')}</th>
                  <th className="portfolio-col-highlight" onClick={handleReturnSort}>Return {returnMode === 'total' ? '(Total)' : '(1D)'} {sortMarker(sortBy)}</th>
                  <th className="portfolio-col-highlight">Action</th>
                </tr>
              </thead>
              <tbody>
                {sortedData.map(([symbol, pos]) => {
                  ...
                  const maxQty = getMaxQty(pos);
                  const selectedSellQty = getSellQty(symbol, maxQty);
                  const lotSize = isOption ? getLotSize(pos.index) : 1;
                  return (
                    <tr key={symbol}>
                      <td data-label="Symbol">
                        <div className="portfolio-symbol">
                          ...
                          <div>
                            <strong>{symbol} <span className="portfolio-col-badge">{pos.type || 'EQUITY'}</span></strong>
                            {isOption ? (
                                <span>{pos.expiry} | {pos.strike}</span>
                            ) : (
                      ...
                      <td data-label="Qty">
                        {isOption ? `${pos.lots || 0} lots (${pos.qty || 0} contracts)` : (pos.qty || 0)}
                      </td>
                      ...
                      <td data-label="Action">
                        <div className="portfolio-sell-action">
                          <input
                            ...
                            onChange={(e) => updateSellQty(symbol, e.target.value, maxQty)}
                            className="portfolio-sell-input"
                          />
                          {isOption && (
                            <small style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-secondary)', marginTop: '2px' }}>
                              Lots (1 lot = {lotSize} contracts)
                            </small>
                          )}
